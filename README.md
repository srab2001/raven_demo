...
- Demo 4 (proposed, not yet built): Explainability-as-a-Service (XaaS) Fabric — a first-class microservice any RAVEN feature calls before showing a Veteran or caseworker a recommendation, returning the rules matched, the source records behind them, a conformal confidence interval, live subgroup fairness metrics, and an "I disagree" path back to the model owner and caseworker. See [docs/XAAS_STRATEGY.md](docs/XAAS_STRATEGY.md) for the strategy and the "DEMO 4" section of [CLAUDE_CODE_BUILD_BRIEF.md](CLAUDE_CODE_BUILD_BRIEF.md) for the build spec.
